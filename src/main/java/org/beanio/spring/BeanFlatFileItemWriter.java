...
public class BeanFlatFileItemWriter<T> implements ItemStream, ItemWriter<T>, ResourceAwareItemWriterItemStream<T>, InitializingBean {
    ...
    /*
     * (non-Javadoc)
     * @see org.springframework.batch.item.file.ResourceAwareItemWriterItemStream#setResource(org.springframework.core.io.WritableResource)
     */
    @Override
    ...
    private class Stream {
        ...
        private Writer createBufferedWriter(FileChannel fileChannel, String encoding) {
            try {
                ...
                if (transactional) {
                    final TransactionAwareBufferedWriter output = new TransactionAwareBufferedWriter(fileChannel, new Runnable() { 
                        public void run() { 
                            Stream.this.destroy();
                        }
                    });
                    ...
                    writer = new BufferedWriter(Channels.newWriter(fileChannel, encoding));
                }
